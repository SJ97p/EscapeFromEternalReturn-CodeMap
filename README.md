...
- 금지구역, 하이퍼루프, 지역 이벤트처럼 다른 팀원이 작업한 지역 기반 기능이 `Zone` 상태/API 위에 붙을 수 있도록 확장 지점 제공
...
| Zone State API / Collaboration Extension | 금지구역, 하이퍼루프, 지역 이벤트가 Zone 내부 구현에 직접 의존하면 협업 확장이 어려움 | `SetZoneState`, `SetZonesState`, `OnZoneStateChanged`로 지역 상태 API 분리 | [Zone State API](docs/systems/zone-state-api.md) |
