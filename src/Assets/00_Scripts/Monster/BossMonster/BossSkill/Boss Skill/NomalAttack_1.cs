...
{
    [CreateAssetMenu(menuName = "Monster/BossMonsterSkill/NomalAttack_1Skill")]
    public class NomalAttack_1 : BossMonsterSkill
    {
        ...
        public override void OnDamage(BossContext con, BossMonsterSkillSlot skillSlot)
        {
            if (skillSlot.hitboxComponent.Target != null && skillSlot.hitboxComponent.Target.TryGetComponent(out IDamageable damageable))
            {
                ...
                if (con.combat.IsBackAttack(con.target.transform, con.owner.transform) && con.phase >= 2)
                {
                    damage *= 1.5f;
